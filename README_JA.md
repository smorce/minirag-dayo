...
使用するブランチ：feature-metadata-filtering 

一旦、フィルター機能は完成したように見える。
→これはポスグレでのテストはしていなくて、インメモリでのテストでしている。(メタデータフィルターありのMiniRAGをやってみる.ipynb)
...
補足:
- 現在の設定でGPU版は解決されます（`[[tool.uv.index]]` cu121 + `sources` マッピング済み）。警告回避のため、依存を `torch==2.5.1+cu121` に微修正も可能です。希望ならすぐ直します。

短い状況メモ:
- `pyproject.toml` をGPU用に調整し、`uv lock` は成功。次に `uv sync` も完了。


### Codexによる修正：メタデータフィルターで light モードと mini モード はフィルターが効いてなかったので以下のように修正してもらった

_filter_items_by_metadata_を導入し、ライトモードのローカルおよびグローバルコンテキストビルダー内で適用しました。これにより、プロンプトを構成する前に、要求されたメタデータ外のエンティティとリレーションシップが除去されます。

テキストユニットコレクターとミニモードコンテキストパイプラインでチャンクメタデータキャッシュを再利用しました。これにより、繰り返されるチャンク検索がフィルタを尊重しつつ、冗長なストレージアクセスを回避します。

軽量スタブを介して、ライトモードとミニモードの両方におけるメタデータフィルタ動作をカバーする回帰テストを追加しました。
